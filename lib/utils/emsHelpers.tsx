...
export function parseStoredEMSCase(stored: string | null): IEMSCaseEntry | null {
  if (!stored) return null;
  try {
    return JSON.parse(stored) as IEMSCaseEntry;
  } catch (error) {
...
export function replacePronounInNode(node: ReactNode, replacement: string): ReactNode {
  if (typeof node === "string") {
    return node.replace(/\*\*pronoun\*\*/gi, replacement);
  }

  if (Array.isArray(node)) {
    return node.map((child, index) => replacePronounInNode(child, replacement));
  }

  if (isValidElement(node)) {
    const element = node as ReactElement<any>; // 👈 key fix

    const newChildren = replacePronounInNode(element.props.children, replacement);
