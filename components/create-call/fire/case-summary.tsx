...
export default function FireCaseSummary({
  ...
  const [sceneStatus, setSceneStatus] = useState<string>(
    fireCase.secureScene ? "Secure" : "Not Secure"
  );
  const [stagingLocation, setStagingLocation] = useState<string>("N/A");
  const [unitsToSend, setUnitsToSend] = useState<string[]>(
    fireCase.unitsToSend && fireCase.unitsToSend.length > 0
      ? fireCase.unitsToSend.filter((unit) => unit !== "Pending")
      : []
  );

  // Get available units from localStorage (similar to unit table)
  const [availableUnits, setAvailableUnits] = useState<string[]>([]);

  // Function to automatically select units based on response plan and postal run order
  const selectUnitsFromResponsePlan = (responsePlan: any, availableUnits: string[]) => {
    ...
    const findBestUnitWithProximityOverride = (unitType: string, quantity: number, alreadySelected: string[]): string[] => {
      ...
      for (let i = 0; i < quantity; i++) {
        ...
        for (let priorityIndex = 0; priorityIndex < priorityOrder.length; priorityIndex++) {
          ...
          const availableOfThisType = availableUnits.filter(unit => {
            ...
            if (alreadySelected.includes(unit) || selectedForThisType.includes(unit)) return false;

            // Check if this unit matches the priority type
            if (priorityType === "AMR 400 series") {
              return unit.includes("AMR") && (unit.includes("4") || unit.includes("400"));
            }
            if (priorityType === "AMR 600 series") {
              return unit.includes("AMR") && (unit.includes("6") || unit.includes("600"));
            }
            if (priorityType === "AMR 800 series") {
              return unit.includes("AMR") && (unit.includes("8") || unit.includes("800"));
            }
          ...
          for (const unit of availableOfThisType) {
            ...
            if (priorityIndex === 0 || position < bestPosition) {
              ...
              if (bestUnit === null || 
                  ...
                  (bestPosition > 3 && position <= 3 && priorityIndex < bestPriorityIndex + 2)) {
                bestUnit = unit;
    ...
    responsePlan.units.forEach((requiredUnit: { type: string; quantity: number }) => {
      ...
      const unitsForThisType = findBestUnitWithProximityOverride(unitType, quantity, selectedUnits);
    ...
    return units.sort((a, b) => {
      ...
      const getPriorityIndex = (unitName: string) => {
        ...
        for (let i = 0; i < unitPriorityOrder.length; i++) {
          ...
          if (unitName.startsWith(priorityType + " ") || unitName.includes(priorityType)) {
            return i;
          }
        }
  ...
  const handleRefreshUnitSelection = () => {
    if (availableUnits.length > 0 && responsePlan) {
      const recommendedUnits = selectUnitsFromResponsePlan(responsePlan, availableUnits);
  ...
  return (
    <div>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => {
                  ...
                  fireCase.secureScene = value === "Secure";
                }}
      ...
      <div className="flex justify-end space-x-2 pt-4">
        ...
        >
          {fireCase.hasCompletedDisconnect
            ? "Close Case"
            : fireCase.questionsCompleted
            ? "Dispatch Case"
            : "Send Case"}
        ...
        {(settings.multiService &&
          ...
          (shouldSendEMS && (
            <Button
              variant="outline"
              onClick={() => handleClick(summaryText, "ems", unitsToSend)}
            >
